...
import { ElasticSearchResponse } from 'abstraction/other/type';
...
async function fetchPRComments(
  ...
  try {
    ...
    do {
      ...
      prFormattedData = await searchedDataFormator(getPrData as ElasticSearchResponse);
      await Promise.all(
        prFormattedData.map(async (prData) => {
          sqsClient.sendMessage(
            { prData, owner, repoName },
            Queue.qGhPrReviewCommentMigration.queueUrl,
            { requestId, resourceId: prData.githubPRId as string }
          );
        })
      );
      from += size;
