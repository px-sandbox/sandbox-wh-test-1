...
const processCommits = async (
  payload: Github.ExternalType.Webhook.WorkbreakdownMessage
): Promise<void> => {
  logger.info({
    message: 'processCommits.invoked',
    data: { payload },
    requestId: payload.reqCtx.requestId,
  });
  ...
  await elasticsearchClient.putDocument(GithubIndices.GitCommits, updatedData);

  deleteProcessfromDdb(processId, { requestId, resourceId });
  logger.info({
    message: 'processCommits.completed',
    data: { payload },
    requestId: payload.reqCtx.requestId,
  });
};
