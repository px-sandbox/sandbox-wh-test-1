...
export const handler = async function updateMergeCommitDataReceiver(event: SQSEvent): Promise<void> {
        ...
        event.Records.map(async (record: SQSRecord) => {
            ...
            try {
                ...
                if (parentCommit) {
                    ...
                    if (!validatedData) {
                        logger.error('updateMergeCommitFormattedDataReceiver.error', { error: 'validation failed' });
                        return;
                    }
