...
import { createSprintQuery, getDateRangeQueries, sprintHitsResponse } from './get-sprint-variance';
...
async function getGraphData(
    ...
    const query = esb
        ...
        .query(
            esb
                .boolQuery()
                .must([esb.termsQuery('body.sprintId', sprintIdsArr), esb.termQuery('body.isDeleted', false)])
        )
        .toJSON();
...
export async function getTimeSpentGraphData(
    ...
    try {
        ...
        for (const bucket of buckets) {
            formattedGraphData[bucket.key] = totalTime ? Math.round((bucket.total_time.value / totalTime) * 100) : 0;
        }
